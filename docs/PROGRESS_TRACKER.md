...
| E2E: packages.e2e-spec.ts - 17 tests covering agency CRUD with real media upload, public browse/filtering, active-vs-inactive visibility, link/unlink, duplicate rejection, cross-account ownership guards | ? verified green 2026-09-12 as part of the 239/239 E2E suite |
...
| E2E: `campaigns.e2e-spec.ts` - 14 tests: create with a real uploaded photo, date validation, photo-cap rejection, list mine, private-campaign visibility, view-count increment, edit/delete ownership + persistence, top-contributors empty, public browse excludes private, creatorId/search filters | ? verified green 2026-09-12 as part of the 239/239 E2E suite |
...
| `POST /campaigns/:id/donate-manual` (traveler auth, `Idempotency-Key`, self-donation rejection) - labeled manual-funding demo, calls `WalletService.recordDonation` atomically | ? built |
...
| Agency subscription billing (#28) | ? RESOLVED 2026-09-12 - RevenueCat native IAP on iOS/Android; backend webhook verifies envelope + X-RevenueCat-Webhook-Signature, dedupes via Redis, updates Agency.subscriptionTier |
...
| E2E: `test/wallet.e2e-spec.ts` - 9 describe blocks covering `GET /me/wallet`, `GET /me/wallet/transactions` (cursor pagination + `?type=` filter), admin credit (auth/role/404/idempotency/currency-mismatch/amount validation/happy path/replay same-body/replay different-body returns original), withdrawal request (insufficient/currency/no-Idempotency-Key/happy path with no debit/agency caller/role block), my withdrawals (empty/own-only/detail/404 cross-user), admin review (approve/reject/non-requested 422/role block/no-Idempotency-Key), admin mark-paid (happy path with debit + walletTransactionId/422 from requested/422 from already-paid/no-Idempotency-Key/role block), transaction list filters + cross-user isolation, role guards (admin blocked from /me/wallet, traveler blocked from /admin/wallet) | ? 41 tests - verified green 2026-09-12 as part of the 239/239 E2E suite |
| Full backend unit suite (incl. donate-manual coverage): 539/539 passed, all green; repo-wide lint/build + full E2E suite now GREEN - the `CallsService` DI blocker is RESOLVED |
...
**Manual-donation demo (backend + mobile, 2026-09-11)**

A labeled, self-contained demo path for manual funding - NOT a real payment processor. It exercises the full wallet-credit flow end to end without external money movement.

**Backend**

| Endpoint | Status |
|---|---|
| `POST /campaigns/:id/donate-manual` (traveler auth) | ? built |

- Requires `Idempotency-Key` header (8-128 chars, validated by `IdempotencyKeyGuard`).
- Rejects self-donation (donor === campaign creator) with a 422.
- Calls `WalletService.recordDonation` atomically inside one Prisma `$transaction`: writes the `Donation` row, credits the creator `WalletAccount` via `WalletTransaction` (direction `credit`, type `donation_received`), and increments `Campaign.raisedAmount`. Idempotency is enforced by the same unique-key short-circuit as every other wallet-mutating write - replaying the same key returns the original transaction instead of double-writing.
- Labeled demo path only: no `IFundingProvider` call, no external transfer. It exists to validate the ledger primitives and the mobile end-to-end flow ahead of the real funding rail.


**Mobile - screens/routes (wallet + withdrawal, 2026-09-11)**

| Screen/Route | Status |
|---|---|
| `app/(traveler)/wallet` - wallet hub (balance, currency, recent transactions, "Withdraw" CTA) | ? built |
| `app/(traveler)/wallet/withdrawals/new` - withdrawal request form (amount, optional campaignId, Idempotency-Key) | ? built |
| TravelerBottomNav - "Wallet" tab (4th item) links to /wallet; Wallet, Transactions, Withdrawals all reached here (not only Settings) | ? built |
| Settings screen - "Wallet" link wired to the wallet route (remains as secondary entry point) | ? built |
| `app/(traveler)/campaigns/[id].tsx` - DonateSheet as a modal (not a separate route), client-generated Idempotency-Key, invalidates campaign/campaigns on success | ? built |

**Mobile - api & hooks**

| File | Status |
|---|---|
| `src/api/wallet.ts` - `getMyWallet`, `listTransactions`, `listMyWithdrawals`, `createWithdrawal`, `getWithdrawalDetail`, `donateManual` | ? built |
| `src/api/types.ts` - adds `DonateManualInput`, `DonateManualResponse`, `WalletAccount`, `WalletTransaction`, `WalletTransactionsPage`, `WithdrawalRequest`, `WithdrawalsPage`, `CreateWithdrawalInput`; adds `raisedAmount` on `Campaign` | ? built |
| `src/hooks/use-wallet-queries.ts` - `useMyWallet`, `useWalletTransactions`, `useMyWithdrawals`, `useWithdrawalDetail` | ? built |
| `src/hooks/use-wallet-mutations.ts` - `useCreateWithdrawal`, `useDonateManual` | ? built |

**Verification**

| Check | Result |
|---|---|
| Backend unit suite | prior full-suite 539/539 passed, all green (now superseded by focused checks) |
| Backend repository-wide lint/build | GREEN as of 2026-09-12 - the `CallsService` DI blocker is resolved; `npm run build` exits 0 with zero TS errors |
| Backend wallet E2E | prior 239/239 E2E suite green; wallet production regression E2E 1/1 passed (new focused test) |
| Mobile `npm run lint` | passed, only pre-existing warnings |
| Mobile `npm run typecheck` | passed |
| Mobile RevenueCat tests | 44 passed |
| Mobile RevenueCat focused lint | passed (repo-wide lint/typecheck still blocked by pre-existing malformed e2e/app.spec.ts and unrelated lint debt) |
| Expo web build | passed |
| Mobile `npm test -- --runInBand` | no tests exist; exited 1 - **no mobile tests exist**, not a pass |
...
| Identity verification (#3 / #5) | Deferred |
| Agency subscription billing (#28) | RESOLVED 2026-09-12 - RevenueCat native IAP (backend webhook + mobile react-native-purchases); not the wallet ledger |
...
| #28 | Subscription tier purchase channel | RESOLVED 2026-09-12 - RevenueCat native IAP on iOS/Android; backend webhook + mobile react-native-purchases; Stripe is NOT the subscription path |
...
| E2E (`group-campaigns.e2e-spec.ts`, 15 tests): privacy forced + admin seeded, never in public browse, non-friend/non-admin add rejected, group chat created on first add and kept in sync, non-member blocked from every group endpoint, contribution self-attribution + overview math, expense lifecycle + admin-only delete, paidByUserId-must-be-member, `group-trips/mine` listing | ? verified green 2026-09-12 as part of the 239/239 E2E suite |
